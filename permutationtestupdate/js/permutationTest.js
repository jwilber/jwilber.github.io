...
var width = svgD3.node().getBoundingClientRect().width;
var height = svgD3.node().getBoundingClientRect().height;
var margin = 20;
var mobileWidth = 390;
var roundPath =
...
var trtCenter = width / 5;
var cntrlCenter = width / 1.5;
var heightMuCenter = height / 1.8;
var scaleWidth = width > mobileWidth ? 0.6 : 0.35;

//////////////////////////
///// node functions /////
//////////////////////////

var nodeTreatmentWidth = (d) => {
...
var nodeGroupInitialForceCollide = (d) => {
...
var nodeGroupMoveForceCollide = (d) => {
	if ((d.nodeGroup === 'llama') & (width > mobileWidth)) {
		return 29;
	} else if (d.nodeGroup === 'llama') {
		return 14;
...
var nodeGroupMoveForceCollideUp = (d) => {
	if (d.nodeGroup == 'resp') {
		return 15;
	} else if (d.nodeGroup == 'llama') {
		return 37;
...
var centPositions = { x: width / 2, y: height / 3 };

var roleScale = d3.scaleOrdinal().range(['coral', 'olive', 'skyblue']);

// dsn dots need to live off-screen, until the final dsn build (so don't mess w/ other nodes)
var sampleData = d3.range(250).map((d, i) => ({
	r: 40 - i * 0.5,
	value: width / 2 + d3.randomNormal(0, 1.5)() * 50,
	nodeGroup: i <= 23 ? 'llama' : i <= 39 ? 'resp' : 'dsn',
	dotValue:
		i % 2 === 0 ? d3.randomNormal(7, 2.5)().toFixed(1) : d3.randomNormal(4.5, 0.75)().toFixed(1),
	permDsn: d3.randomNormal(0, 1)().toFixed(1)
}));

var center = d3.forceCenter().x(centPositions.x).y(centPositions.y);

var manyBody = d3.forceManyBody().strength(1);
...
d3.html('./permutationtest/assets/noun_28240_cc.svg', loadRoughsvgD3);
...
function shuffvarestStat(nodePositions, responseNode) {
...
var treatmentTitleCenter = trtCenter;
var controlTitleCenter = cntrlCenter;
var treatmentTitle = svgD3
	.append('text')
	.html('TREATMENT')
	.attr('x', trtCenter - 30)
...
var dotDistRangeStart = width > mobileWidth ? width / 4 : width / 8;
var dotDistRangeEnd = width > mobileWidth ? width / 1.5 : width / 1.15;
var dotDistHeight = width > mobileWidth ? height : height + height / 1.05;
...
function dotDistribution() {
	var data = sampleData.filter((d) => d.nodeGroup === 'dsn');
	...
	var binContainer = svgD3.append('g').selectAll('.gBin').data(bins);

	var binContainerEnter = binContainer
		.enter()
		.append('g')
		.attr('class', 'gBin')
	...
	binContainerEnter
		...
		.attr(
			'transform',
			(d) => `translate(-20, ${-d.idx * 2 * d.radius - d.radius - dotDistHeight / 8})`
		);
} //update
...
function initRoughDistribution() {
	...
	d3.selectAll('.distributionCircleG').selectAll('path').style('fill', 'pink').style('opacity', 1);
}
...
function transitionZeroUp() {
	...
	d3.selectAll('.dot').select('path').transition().style('fill', 'rgba(131, 131, 131, .05)');
}
...
var responseTextSize = width > mobileWidth ? '0.95rem' : '0.5rem';
var responseTextX = width > mobileWidth ? 1.8 : 1;
var responseTextY = width > mobileWidth ? 45.2 : 24;
...
var distributionDotScale = width > mobileWidth ? 0.11 : 0.055;
...
function transitionFourUp() {
	shuffvarestStat(nodeRandomPosTwo, '.response3');
...
function transitionFiveUp() {
	...
	force
		...
		.on('tick', changeNetwork);
	force.alpha(0.1).restart();
	shuffvarestStat(nodeRandomPosFive, '.response6');
}
...
function transitionFiveDown() {
	...
	loop(
		...
		function () {
			shuffvarestStat(nodeRandomPosThree, '.response4');
		},
		function () {
			shuffvarestStat(nodeRandomPosFour, '.response5');
		},
		function () {
			shuffvarestStat(nodeRandomPosFive, '.response6');
		},
		function () {
			shuffvarestStat(nodeRandomPosSix, '.response7');
		},
		function () {
			shuffvarestStat(nodeRandomPosSeven, '.response8');
		},
		function () {
			shuffvarestStat(nodeRandomPosEight, '.response9');
		}
	);
}
...
function transitionSixUp() {
	...
	d3.selectAll('.distributionCircleG.extreme').selectAll('path').style('fill', 'pink');
}

// hacky way to ensure smallest node keeps relative size (idk why this is a problem, but this solves it!)
var small_node_size_force = 5.5;
...
var finalTextSize = width > mobileWidth ? 20 : 14;
...
function transitionEightDown() {
	...
	var splitValue = width > mobileWidth ? 75 : 20;
	...
	d3.selectAll('.distributionCircleG.extreme')
		...
		.attr(
			'transform',
			(d) => `translate(${splitValue}, ${-d.idx * 2 * d.radius - d.radius - dotDistHeight / 8})`
		);
}
